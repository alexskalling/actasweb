...
async function runMigration() {
  ...
  try {
    console.log('📄 Leyendo archivo de migración 009...');
    const migrationPath = join(process.cwd(), 'lib/db/migrations/009_add_soporte_to_actas.sql');
    const migrationSQL = readFileSync(migrationPath, 'utf-8');

    console.log('🚀 Ejecutando migración 009...');
    await sql.unsafe(migrationSQL);
    
    console.log('✅ Migración 009 ejecutada correctamente');
  } catch (error: any) {
