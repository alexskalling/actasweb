...
async function runMigration() {
  ...
  try {
    console.log('📄 Leyendo archivo de migración 008...');
    const migrationPath = join(process.cwd(), 'lib/db/migrations/008_add_url_contenido_to_actas.sql');
    const migrationSQL = readFileSync(migrationPath, 'utf-8');

    console.log('🚀 Ejecutando migración 008...');
    await sql.unsafe(migrationSQL);
    
    console.log('✅ Migración 008 ejecutada correctamente');
  } catch (error: any) {
